...
def Compete(y_proba1, y_proba2 = y_dummy, margin1 = 1, margin2 = 1):
    ...
    print ('2: Profit (average): ', np.sum(np.select([selector2], [profit2])) / np.sum(selector2), ', deals: ', np.sum(selector2) / selector2.shape[0])
...
if MLP == 1:
    ...
    mlp = MLPClassifier(solver=grid.best_estimator_.solver, alpha=1e-5, hidden_layer_sizes=grid.best_estimator_.hidden_layer_sizes, random_state=11, verbose=1)
